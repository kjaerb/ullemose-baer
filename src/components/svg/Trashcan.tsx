...
interface TrashcanProps extends React.ComponentProps<"svg"> {
  disabled: boolean;
}

export function Trashcan({ disabled, className, ...props }: TrashcanProps) {
  return (
    <svg
      {...props}
      width='24'
      ...
      fill='currentColor'
      className={cn(
        "text-black hover:text-red-600 transition-colors duration-200 cursor-pointer",
